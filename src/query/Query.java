...
public class Query {
	...
	q.nameEquivalence();
	q.structuralEquivalence();
	q.internalEquivalence();	   
	...
	{	
		...
		int check;
			...
			if((G==1)||(G==2))
			{if(curr[X][Y]==1)
			System.out.println("Yes, it is checked for structural equivalence and holds.");
			else
			System.out.println("No, it is checked for structural equivalence and does not hold.");
			}
	...
	public void nameEquivalence()
	{
		...
		if(G==0){
		if(curr[X][Y]==1)
			System.out.println("Yes, it is checked for name equivalence and holds.");
			else
			System.out.println("No, it is checked for name equivalence and does not hold.");
	...
	public void internalEquivalence()
	{
		...
		for(i=0;i<n;i++)
			...
			if(G==3)
			{if(curr[X][Y]==1)
			System.out.println("Yes, it is checked for internal name equivalence and holds.");
			else
			System.out.println("No, it is checked for internal name equivalence and does not hold.");
	...
	public void query() throws IOException
	{
		...
		int k;
		for(k=0;k<n;k++)
		{if(variables.get(k).name().equals(one))
			X = k;
		if(variables.get(k).name().equals(two))
			Y = k;
                }
